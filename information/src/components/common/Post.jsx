...
const Post = ({ post }) => {
	...
	const updatePostInCache = (updates) => {
		queryClient.setQueriesData({ queryKey: ["posts"] }, (oldData) => {
			if (!Array.isArray(oldData)) return oldData;
			return oldData.map((cachedPost) =>
				cachedPost._id === post._id ? { ...cachedPost, ...updates } : cachedPost
			);
		});
	};
	...
	const { mutate: likePost, isPending: isLiking } = useMutation({
		...
		onSuccess: (updatedLikes) => {
			...
			updatePostInCache({ likes: updatedLikes });
		},
		onError: (error) => {
	...
	const { mutate: repostPost, isPending: isReposting } = useMutation({
		...
		onSuccess: (updatedReposts) => {
			updatePostInCache({ reposts: updatedReposts });
			toast.success(updatedReposts.includes(authUser._id) ? "Post reposted" : "Repost removed");
		},
	...
	const { mutate: bookmarkPost, isPending: isBookmarking } = useMutation({
		...
		onSuccess: ({ bookmarked }) => {
			updatePostInCache({ isBookmarked: bookmarked });
				...
				<div className='flex flex-col flex-1'>
					...
					<div className='flex justify-between mt-3'>
						...
						<div className='flex w-1/3 justify-end gap-2 items-center'>
							<button
								type='button'
								className='cursor-pointer text-slate-500 hover:text-primary disabled:cursor-wait'
